...
class AuthController {
  static signup(req, res, next) {
    ...
    if (!email || !rawPassword || !name || !ssn || !pin) {
      return res
        .status(422)
        .send({ error: "You must provide all relevant information" });
    }

    User.validateEmail(email).then(data => {
      if (data.length > 0) {
        return res.status(422).send({ error: "Email address already exists" });
      } else {
        User.validateSSN(ssn).then(data => {
          if (data.length > 0) {
            return res.status(422).send({ error: "SSN already exists" });
          } else {
            return User.createUser(email, password, name, ssn, pin).then(
              data => {
